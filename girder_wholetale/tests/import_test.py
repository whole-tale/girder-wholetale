...
def _testTaleImportZip(self, mock_builder):
...
@mock.patch("girder.plugins.wholetale.lib.manifest.ImageBuilder")
def _testTaleImportZipWithRuns(self, mock_builder):
    mock_builder.return_value.container_config.repo2docker_version = (
        "craigwillis/repo2docker:latest"
    )
    mock_builder.return_value.get_tag.return_value = "some_image_digest"
...
def _test_binder_heuristics(self):
...
def _test_bdbag_import(self):
    with open(os.path.join(DATA_PATH, "Reporter_Cell_Line_14-3QDC.zip"), "rb") as fp:
        resp = self.request(
