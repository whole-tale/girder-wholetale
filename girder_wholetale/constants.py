...
class PluginSettings:
    ...
    # Whether the data manager's periodic file GC runs at all. Its
    # candidate query ({"dm.cached": True, "dm.lockCount": 0}) is served by
    # no index, so every pass is a full scan of the item collection --
    # affordable on a deployment that uses private storage, pure waste on
    # one that does not.
    GC_ENABLED = "dm.gc_enabled"
